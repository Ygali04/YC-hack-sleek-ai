...
func _process_user_input():
	...
	var preamble = "First, write a concise explanation (2-4 sentences). Then output a [gds_actions]...[/gds_actions] block with one command per line using ONLY these verbs and exact signatures:\n- create_file(\"res://path\") + a fenced code block with '# New file: res://path' and file contents\n- create_scene(\"res://path.tscn\", \"RootName\", \"RootType\")\n- create_node(\"Name\", \"NodeType\", \"res://scene.tscn\", \"ParentPath\", { property: value })\n- edit_node(\"NodeName\", \"res://scene.tscn\", { property: value })\n- add_subresource(\"NodeName\", \"res://scene.tscn\", \"SubResType\", { property: value })\n- edit_subresource(\"NodeName\", \"res://scene.tscn\", \"SubResPropName\", { property: value })\n- assign_script(\"NodeName\", \"res://scene.tscn\", \"res://script.gd\")\n- edit_script(\"res://script.gd\") with the updated content included in a fenced code block\n- create_image({ prompt: string, aspect_ratio: string (use '1:1' for spritesheets), seed: int, output_format: string ('png' or 'jpeg'), output_prefix: string, exact_output_path: string })\n- spritesheet_to_spriteframes(\"NodePath\", \"res://scene.tscn\", { texture: \"res://path.png\", rows: int, cols: int, frame_width: int, frame_height: int, animations: [ { name: string, start: int, length: int, speed: float, loop: bool } ], assign_to_property: \"sprite_frames\" })\n- create_2d_char(\"prompt\", { rows: int, cols: int, animation: string, fps: float, loop: bool, run: bool })  // power-action that performs the full 2D character pipeline in the OPEN SCENE\n- run_project()  // plays the main scene\nDo NOT invent other verbs (e.g. edit_project_settings). Close the actions with [/gds_actions].\n\nMUST CALL FS SNAPSHOT: Include get_fs_markdown({ path: 'res://', max_depth: 3 }) as the FIRST action and BETWEEN EVERY action in the block.\n\nCHARACTER PIPELINE (STRICT): Prefer create_2d_char. Do NOT create a new scene file when building a character; operate in the currently open scene. The image must be a 16-bit 2D pixel-art spritesheet, ordered left-to-right, top-to-bottom, with no margins. Use aspect_ratio '1:1'.\n\nProhibited: create_node(..., \"AnimatedSprite2D\", ..., \".\", ...) — never parent AnimatedSprite2D to '.'; parent it to the CharacterBody2D created in the same block.\n\nCorrect example:\n[gds_actions]\nget_fs_markdown({ path: 'res://', max_depth: 3 })\ncreate_2d_char(\"16-bit 2D pixel art gray tabby cat jumping\", { rows: 3, cols: 3, animation: \"cat_jump\", fps: 8, loop: true, run: true })\n[/gds_actions]\n\nIf create_2d_char is unavailable, fall back to explicit steps ONLY if a scene path is known; otherwise ask the user to open a scene. The explicit order is: (1) get_fs_markdown, (2) create_node CharacterBody2D under the scene root, (3) get_fs_markdown, (4) create AnimatedSprite2D as its child (parent=CharacterBody2D), (5) get_fs_markdown, (6) create_image with aspect_ratio '1:1', (7) get_fs_markdown, (8) spritesheet_to_spriteframes targeting the AnimatedSprite2D node, (9) get_fs_markdown, (10) set animation to autoplay, (11) get_fs_markdown, (12) create Camera2D as a child of the character and make it current, (13) get_fs_markdown, (14) run_project().\n\n"
	var ci = ""
	if ai_session_manager:
...
func _append_assistant_to_current_attempt(text: String, metadata: Dictionary) -> void:
	...
	# Parse and render actions from the raw text under this attempt's AI container
	var actions = actions_manager.parse_actions_from_text(text, 0)
	if actions.size() > 0:
		actions_manager.render_actions(actions, ai_container)
		# Ensure the newly added buttons are visible
		await get_tree().process_frame
		scroll.scroll_vertical = scroll.get_v_scroll_bar().max_value
		rating_container.visible = true
